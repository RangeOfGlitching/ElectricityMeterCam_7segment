...
RYLR896::RYLR896(HardwareSerial* loraSerial, hardSerialConfig_t config){
    // Initialize Serial
    this->loraSerial = loraSerial;
    this->loraSerial->begin(config.baudRate, config.serialMode, config.rxPin, config.txPin); 
    this->SetIPR(config.baudRate);
}
...
bool RYLR896::SetBand(int band){
    this->WriteToLoRa("AT+ADDRESS="+String(band));
    String response = ReadFromLoRa();
    if (response == "+OK") {
        return true;
    } else return false;
...
bool RYLR896::SetAESPassword(String password){
    if (password.length() != 32) return false;
    this->WriteToLoRa("AT+CPIN="+String(password));
    String response = ReadFromLoRa();
    if (response == "+OK") {
        return true;
    } else return false;
